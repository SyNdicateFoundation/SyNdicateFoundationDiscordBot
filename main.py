...
async def HElP(ctx: commands.Context):
    ...
    embed = discord.Embed(title=f"Help", description=f">  ⚠️Help\n"
                                                        f"سلام {name}\n"
                                                        f"development_team\n"
                                                        f"clear\n"
                                                        f"ping\n"
...
async def send(ctx, *, message=None):
    ...
    if message == None:
        ...
        embed2 = discord.Embed(title=f"A message From {ctx.author.guild}", colour=discord.Colour.blue())
        embed2.add_field(name="Message :", value=f"{message}")
        embed2.set_thumbnail(url=ctx.guild.icon_url)
        await ctx.send(embed=embed2)
